...
public class Situador : MonoBehaviour
{
    ...
    public float Tiempo_dia = 360f;
    //public float Ultimo_tiempo_sobrante = -1;

    public bool ActivarIA = true;

    private Tiempo tiempopoDia;

    // Use this for initialization
    void Start()
    {

       // Debug.Log(Tiempo.tiempoSobra);
        tiempopoDia = GameObject.Find("Player").GetComponent<Tiempo>();
        ...
        if (GestionDias.dia==1 || !ActivarIA)//Si es el primer dia no habrá datos del último tiempo sobrante, por tanto situaremos el número de zombis por defecto
        {
        ...
        else //Para el resto de días calculamos cuántos zombis poner
        {
            ...
            if (Tiempo.tiempoSobra > mitad_tiempo&& !Caza.muerto)// 
            {
                Debug.Log("Máximo");
            ...
            {
                Caza.muerto = false;
                Zombis_que_se_van_a_situar = Numero_Zombis_maximo-Numero_Zombis_minimo;
            }
