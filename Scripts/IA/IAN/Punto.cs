...
public class Punto : MonoBehaviour {
    ...
    //Indica si se puede tomar el punto como lugar de inicio de un zombi
    public bool inicializable = true;

    // Use this for initialization
    void Start () {
		Player = GameObject.FindWithTag("Player").transform;
        if (!inicializable) {
            valorProbabilidad = 0f;
        }
	}
	
	// Update is called once per frame
	void Update () {
        if ((Time.time >= nextTime) && inicializable) 
		{
